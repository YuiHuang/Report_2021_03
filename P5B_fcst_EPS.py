...
def fcst_EPS(ticker, year, season, year_m, month):
    ...
    #=== month: 月營收的截止月份
    """
    fn = f"../data/fs_report/fs_{ticker}_20203.csv"
    ...
    for i in range(4):
        ...
        margin4.append(df.loc[four_seasons[i], "毛利率"])
    margin4.reverse()
    ...
    #=== 利益率年增率
    margin_yoy = (sum(margin4) / 4 - margin4[0]) / margin4[0]
    EPS_new = EPS4 * (1 + rev3_yoy) * (1 + margin_yoy) / (1 + cap_yoy)

    print(f"近4季的EPS: {EPS4:.2f}")
    print(f"最近3個月的營收YoY (%): {rev3_yoy * 100:.1f}")
    print(f"利益率YoY (%): {margin_yoy * 100:.1f}")
...
if __name__ == '__main__':
    ...
    year_m = "2016"
    month = "11"
    fcst_EPS("4205", year, season, year_m, month)